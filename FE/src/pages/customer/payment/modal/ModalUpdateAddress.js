...
const ModalUpdateAddress = ({ visible, id, onCancel }) => {
  ...
  const getOne = () => {
    AddressClientApi.detailAddressClient(id).then((res) => {
      form.setFieldsValue({ userId: res.data.data.user.id });
      setAddress(res.data.data);
      setStatusAddress(res.data.data.status);
      form.setFieldsValue(res.data.data);
      AddressClientApi.fetchAllProvinceWard(res.data.data.toDistrictId).then(
        (resWard) => {
          setListWard(resWard.data.data);
        }
      );
      AddressClientApi.fetchAllProvinceDistricts(res.data.data.provinceId).then(
        (resDistrict) => {
  ...
  const handleOk = () => {
    form
      ...
      .then((values) => {
        AddressClientApi.update(id, values)
          .then((res) => {
